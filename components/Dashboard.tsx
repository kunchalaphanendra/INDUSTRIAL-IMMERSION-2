...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout, onBackToLanding }) => {
  ...
  const getStatusInfo = (progress: number) => {
    if (progress < 20) return { label: 'PHASE 01: ONBOARDING', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/30' };
    if (progress < 50) return { label: 'PHASE 02: EXECUTION', color: 'text-purple-500', bg: 'bg-purple-500/10', border: 'border-purple-500/30' };
    if (progress < 80) return { label: 'PHASE 03: MANAGEMENT', color: 'text-orange-500', bg: 'bg-orange-500/10', border: 'border-orange-500/30' };
    return { label: 'MISSION COMPLETE', color: 'text-green-500', bg: 'bg-green-500/10', border: 'border-green-500/30' };
  };
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="glass-card rounded-[3.5rem] p-10 md:p-14 border-white/10 mb-12 flex flex-col lg:flex-row items-center justify-between gap-12 relative overflow-hidden group">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto relative z-10">
            <button onClick={onBackToLanding} className="flex-1 px-8 py-4 bg-white/5 hover:bg-white/10 text-white font-bold rounded-2xl transition-all border border-white/10 uppercase tracking-widest text-xs">Landing Page</button>
        ...
        <div className="space-y-10">
          <div className="flex items-center justify-between px-2">
             <div className="flex items-center gap-3">
               <h2 className="text-xl font-heading font-black tracking-widest uppercase text-white">Your Subscriptions</h2>
             ...
             <div className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">{enrollments.length} Active Records</div>
          </div>
          ...
          {loading && enrollments.length === 0 ? (
            <div className="glass-card p-32 rounded-[3rem] flex flex-col items-center justify-center text-gray-500 border-white/5">
              ...
              <p className="font-heading font-bold uppercase tracking-widest text-xs opacity-50">Connecting to Industrial DB...</p>
            </div>
            ...
            <div className="glass-card p-24 rounded-[3rem] flex flex-col items-center justify-center text-gray-500 border-dashed border-white/10 text-center">
              ...
              <h3 className="text-xl font-heading font-bold text-white mb-2 uppercase">No Enrolled Tracks</h3>
              <p className="max-w-xs mb-8 text-gray-500 text-sm">Once you complete a payment, your active plan will appear here instantly.</p>
              ...
              {enrollments.map(record => {
                ...
                return (
                  <div key={record.id} className="glass-card p-10 rounded-[3rem] border-white/10 hover:border-blue-500/40 transition-all group relative overflow-hidden">
                    ...
                    <div className="absolute top-0 left-0 w-1 h-full bg-blue-600 shadow-[0_0_20px_rgba(37,99,235,0.8)]" />
                    ...
                    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10">
                      <div className="flex-1 space-y-6">
                        <div className="flex flex-wrap items-center gap-4">
                          ...
                          <span className={`px-3 py-1.5 ${status.bg} ${status.color} text-[10px] font-black uppercase tracking-[0.2em] rounded-lg border ${status.border}`}>
                            {status.label}
                          </span>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-x-8 gap-y-2 text-[11px] font-bold text-gray-500 uppercase tracking-widest">
                             <div className="flex items-center gap-2">
                               <svg className="w-4 h-4 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                      ...
                      <div className="w-full lg:w-80 p-8 bg-white/[0.02] border border-white/5 rounded-3xl">
                         <div className="flex justify-between items-end mb-4">
                            <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Immersion Depth</span>
